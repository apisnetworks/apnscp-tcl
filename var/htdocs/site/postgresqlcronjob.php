<?php
	...
	*/
	...
	if (!$pgsqlcronjobEnabled) die("The requested module is disabled.");
	Login();
	
	/* triggers PostgreSQL cronjob creation */
	...
	if (isset($_POST['go'])) {
		...
		while (@list($b,$c) = @each($_POST['name'])) {
			...
			$q = mysql_query("select 1 from pgsql_jobs where domain = '".$gDomainName."' AND name = '".$name."'",$conna);
			...
			if ($reset || !$exists) {
				...
				$q = mysql_query("select time from pgsql_jobs where domain = '".$gDomainName."' AND name = '".$name."'",$conna);
				$row = mysql_fetch_row($q);
				$time = $row[0];
			}
			if ($exists) {
				$q = mysql_query("update pgsql_jobs set notify = '".$notify."', time = '".$time."', email = '".$email."', day_span = '".$perform."', 
					enabled = '".$enable."', type = '".$format."' where domain = '".$gDomainName."' AND name = '".$name."';",$conna);			
			} else {
				$q = mysql_query("insert into pgsql_jobs values('" . $time . "', '".$gDomainName."', '".$email."', '".$perform."',
...
<table width=640 border=0 cellspacing=0 cellpadding=0>
  <tr> 
    <td class=head> <table width=640 border=0 cellspacing=1 cellpadding=1>
        ...
        <tr> 
          <td class=cell1 colspan=2> &nbsp; <a href="mysqlcronjob.php" onMouseOver="status='MySQL Cronjob'; return true;" onMouseOut="status=''; return true;">MySQL 
            Cronjob </a> <img src="/images/red.gif" width=4 height=4> <a href="postgresqlcronjob.php" onMouseOver="status='PostgreSQL Cronjob'; return true;" onMouseOut="status=''; return true;"><strong>PostgreSQL 
            Cronjob </strong></a></td>
        </tr>
      </table></td>
  </tr>
						...
						<table width=640 border=0 cellspacing=0 cellpadding=1>
							<tr> 
								...
								<td width="487" align=left valign="middle" class=cell1>
									<b>Result:</b>&nbsp;PostgreSQL Cronjob successful.
								</td>
							</tr>
						</table> 
					</td>
...
?>
<table width=640 border=0 cellspacing=0 cellpadding=0>
<form method=POST action="postgresqlcronjob.php">
	<tr>
		<td class=head valign=top> 
			<table border=0 cellspacing=1 cellpadding=1 width=100%>
				<tr> 
					<td class=head3 colspan=7>PostgreSQL Cronjob</td>
				</tr>
				<tr> 
					<td class=help valign=center colspan=7>
						PostgreSQL Cronjob will setup periodic dumps of your database,
						inclusive of all tables, into your home directory, i.e.
						<em>/home/<?php print($gUserName); ?></em>/postgres_dumps/pgsql-YYYYMMDD-dbname.ext
						<br>
						Limitations:
						<li>Only one cronjob per database</li>
						<li>The dump of your database will count towards your disk quota</li>
						<li><strong>Only</strong> schema and values are dumped</li>
						<li>When notify is selected, an e-mail will be dispatched to the site administrator
						notifying him/her of the dump</li>
						...
						<li><em>Conversely:</em> on January 10 if you changed 'notify' to no, and the perform
						was set to'10' without the reset checkbox ticked, then the next occurrence will be on
						January 20 without an e-mail notification</li>
						<li>This command is analogous to :<br>
						`pg_dump -d -U <?php print($gUserName); ?> -x -O DBNAME >/home/<?php print($gUserName); ?>/pgsql_dumps/pgsql-YYYYMMDD-dbname.ext`</li>
					</td>
				</tr>
				<tr>
				...
				<?php
					// YOU NEED POSTGRESQL COMPILED INTO PHP FOR THIS
					pg_connect("host=localhost user=".$pgsql['databases']['username']." password=".$pgsql['databases']['password'].
						" dbname=".$pgsql['databases']['database']);
					$pgsqlusers = PostgresqlAliases($gDomainName);
					$i = 0;
					$color = "listeven";
					foreach ($pgsqlusers as $NULL => $user) {
						$q = pg_query("select datname from pg_database where datdba = (select usesysid from pg_user where usename = '".$user."')");
						while ($row = pg_fetch_row($q)) {
							$q2 = @mysql_query("select day_span, notify, type, enabled, time from pgsql_jobs where domain = '".$gDomainName."' AND name = '".$row[0]."'", $conna);
							...
							<tr> 
								...
								<td class=<?php print($color); ?> align=center valign=middle>
									Yes
									<input type=radio name="enable[<?php print($i); ?>]" value=1 <?php ($row2[3]) ? print('CHECKED') : print(''); ?> />
									No
									<input type=radio name="enable[<?php print($i); ?>]" value=0 <?php (!$row2[3]) ? print('CHECKED') : print(''); ?> />
								</td>
								<td class=<?php print($color); ?> align=center valign=middle>
								...
								<td class=<?php print($color); ?> align=center valign=middle>
									Yes
									<input type=radio name="notify[<?php print($i); ?>]" value=1 <?php ($row2[1]) ? print('CHECKED') : print(''); ?> />
									No
									<input type=radio name="notify[<?php print($i); ?>]" <?php (!$row2[1]) ? print('CHECKED') : print(''); ?> value=0 />
								</td>
								<td class=<?php print($color); ?> align=center valign=middle>
